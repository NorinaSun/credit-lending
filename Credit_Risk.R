...
process <- function(df,df_type) {
  ...
  df$costincome = df$MNT_DEMANDE / df$REV_NET
...
objControl <- trainControl(method='repeatedcv', number=3, returnResamp='none', summaryFunction = twoClassSummary, classProbs = TRUE, sampling="up")
...
predictions_prob <- predict(object=model, to_predict_processed, type='prob')

#adding the predictions to the dataframe
to_predict$prediction <- predictions_final
to_predict$prediction_prob <- predictions_prob$no_default
to_predict$prediction_threshold <- ifelse(to_predict$prediction_prob >= 0.455, "no_default","default")

#taking just the yesses
good_loans = to_predict[to_predict$prediction_threshold == "no_default",]
